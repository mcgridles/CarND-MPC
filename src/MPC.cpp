...
class FG_eval {
 public:
  ...
  // Much of this is pulled from the MPC lesson
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    // Multipliers affect how smooth or aggressive the controller is
    // Errors
    for (int t=0; t<N; t++) {
    ...
    for (int t=1; t<N; t++) {
      ...
      // Updated for polynomial of order 3
...
//  size_t i;
  ...
  // Add estimated trajectory for green MPC line in sim
  for (int t=0; t<N-2; t++) {
      output.push_back(solution.x[x_start + t + 1]);
      output.push_back(solution.x[y_start + t + 1]);
  }
